...
public record TextPart(String text, float size, FontStyleEntry style)
{
    ...
    private static final Codec<FontStyleEntry> ENTRY_CODEC = FlareConstants.FLARE_KEY_CODEC.xmap(TextPart::findStyle, FontStyleEntry::key);
    ...
    {
        // TODO: use optionals
        FontStyleEntry fontStyleEntry = BuiltInFlareRegistries.FONT_STYLE.get(key).orElseThrow().value();
        if (fontStyleEntry == null)
        {
            LOGGER.warning("Could not find font style '" + key + "'");
            return BuiltInFlareRegistries.FONT_STYLE.get(UnknownCharacter.STYLE_KEY).orElseThrow().value();
        }
